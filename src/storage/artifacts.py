...
def latest_saved(capability: str, key: Optional[SecretStr] = None) -> Optional[SavedArtifact]:
    """The capability's highest version, or None if nothing is saved.
    ...
    if not files:
        return None
    highest = files[-1][0]
    signing_key = key or env.artifact_signing_key
    tied = [_read(path, version, capability, signing_key) for version, path in files if version == highest]
    return max(tied, key=_preference)


def _read(path: Path, version: Version, capability: str, key: SecretStr) -> SavedArtifact:
    try:
    ...
    return SavedArtifact(path, version, artifact, trusted=agrees)

...
from src.evidence.report import status_badge, write_report
...
def discover_runs(runs_dir: Path) -> list[RunEntry]:
    """Every subfolder of runs_dir with a readable result.json, newest first.
    ...
    entries.sort(key=lambda entry: entry.result.start_time, reverse=True)
    return entries
...
def render_index_html(runs: Sequence[RunEntry]) -> str:
    return _env.get_template("index.html").render(rows=[_row(entry) for entry in runs])
...
def render_index_markdown(runs: Sequence[RunEntry]) -> str:
    ...
    for entry in runs:
        ...
        lines.append(f"| {row['capability']} | {row['mode']} | {row['status_label']} | {summary} | "
